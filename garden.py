...
class PlantPests(Pests):
    def __init__(self, pests_type, quantity, plants):
        super(PlantPests, self).__init__(pests_type, quantity)
        self.pests_type = pests_type
        self.quantity = quantity
        self.plants = plants

    def eating(self):
        print(f'Pests are eating...')
        print(f'Plants before eating: {self.plants.index}')
        self.eat_up()
        print(f'Plants after eating: {self.plants.index}')

    def eat_up(self):
        if self.state == 2 or self.state == 3:
            self.plants.index = self.plants.index - self.quantity
            return self.plants.index

    def cross_the_styx(self):
        self.quantity = self.quantity / 2
        print(f'Gardener poison {self.quantity} pests')
